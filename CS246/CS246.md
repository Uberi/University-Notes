...
A basic C++ program might look like the following:
    ...
    using namespace std; // this puts the std namespace into the global namespace, so std::cout is the same as cout; without this line we have to use std::cout
...
The syntax is similar to C. `cout` is the standard output stream, and `endl` is a constant representing the end of line (a newline).
...
`stdio.h` and such are still available, but should not be used because C++ has its own equivalent ways of accomplishing these tasks. Marmoset will be forbidding these libraries from being used.
