        ...
        (cond [(symbol=? start end) (list end)]
              [else
               (local [(define found-paths
                           (filter cons?
                               (map (lambda (node) (find-route node graph))
                                    (neighbors start))))]
                   (cond [(empty? found-paths) false]
                         [else (first found-paths)]))]))

This works, but it isn't very efficient since we only really care about one possible path, and it would be more efficient to stop searchign when we've found a path already:
    ...
    (define (find-route start end graph)
        (cond [(symbol=? start end) (list end)]
              [else (local [;; find-route-list: (listof Node) Graph -> (union (listof Node) false)
                            ;; Purpose: produces a path leading from one of `nodes` or false if not possible
                            (define (find-route-list nodes)
                                (cond [(empty? nodes) false]
                                      [else (local [(define route (find-route (first nodes) end graph))]
                                                (cond [(cons? route) route]
                                                      [else (find-route-list (rest nodes))]))]))]
                        (cons start (find-route-list (neighbors start))))]))
...
Note that the only part of the function that actually does anything with the graph is `neighbors`. In fact, all we have to do is implement a different `neighbors` function. A neighbor function that returned the list of legal moves given the possible 