...
Starting with the Intermediate Student teaching language, helper functions that supplement a wrapper function need only a contract and purpose.
...
;wip: look into the design recipe updates for mutual recursion
...
Now we can write a substitution function:
    ...
    (define (substitute-list alglist var value)
        (cond [(empty? alglist) empty]
              [else (cons (my-algexp-fn (first alglist) var value)
...
;wip: finish this and move it to the semantic model
