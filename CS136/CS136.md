...
The `a ? b : c` is known as the **ternary operator**. It works like `(if a b c)` in Racket. If `a` is true, it evaluates and returns `b`, otherwise `c`. It short-circuits, so one of `b` or `c` is not evaluated: `x >= 0 ? x : -x` results in the absolute value.

Like in Racket, we must define a function before we can call it. However, C needs to know the type signature of the function