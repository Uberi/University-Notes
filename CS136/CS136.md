...
After we are done with the memory we allocated, we should always free it again so it is available for future allocations. This is done using the `void free(void *p)` function, which returns the allocated memory at `p` back to the heap.
...
In RunC, it is required that every `malloc` is paired with a corresponding `free`.
...
* **Double-free bugs**: using `free` on memory that has already been freed, or memory that was not allocated by `malloc`.
...
We assume that the running time of `realloc` is $O(s)$, where $s$ is `s`. `realloc(NULL, s)` is the same as `malloc(s)`, and `realloc(ptr, 0)` is the same as `free(ptr)`.
...
We can see that for $n + 1$ calls, we have $n + \frac{1}{2}n + \frac{1}{4}n + \ldots + 1 = n\sum_{i = 0}^{\lceil\log_2 n\rceil} \frac{1}{2^i} = n\frac{1 - \left(\frac{1}{2}\right)^{\lceil\log_2 n\rceil + 1}}{1 - \frac{1}{2}} = 2n\left(1 - \frac{1}{2}\frac{1}{2^{\lceil\log_2 n\rceil}}\right) = O\left(n - n\frac{1}{n}\right)\right) = O(n)$ copying operations.
