...
In code, we represent null with `NULL`, and is defined in the `stdlib` module (as well as several others).
...
* `(begin a b ... y z)` evaluates each expression `a` to `z` and discards all resulting values except `z`, which it produces. This is only useful when the expressions have side effects. This is not used very often because each function definition in Racket implicitly uses `begin`, just like `local`.
...
In expressions, we can also use a comma (`,`) to evaluate multiple expressions but produce only the last one. For example, $a == 1 ? (a = 2, 5 + 6) : 7 + 8$. This is very similar in functionality to `begin` in Racket.
