...
2. If the function uses recursion:
    ...
    3. Write the function in terms of a recurrence relation and look up the closed form in the table.
...
The time complexity for this is $\sum_{j = 0}^n \sum_{i = 0}^j O(1) = \sum_{j = 0}^n O(n) = O(n^2)$.
