...
This is a very barebones testing module that can get the job done. It doesn't handle invalid inputs or edge cases, since it is only meant to be used by testers.
...
In fact, RunC has facilities to automate using this sort of interactive testing module.
...
Then `p + q` is invalid, because we **cannot add two pointers**. This is because pointers are often rather large numbers, and could easily overflow if added, resulting in an invalid pointer.
...
This allows pointer arithmetic to satisfy `p - q = -25` given that `q = p + 25`, since `(1000 - 1100) / sizeof((int)0) == -25`. Basically, subtracting two pointers results in the number of slots of the data type that would fit between the two.
...
;wip: the midterm covers everything up to section 7, slide 26
...
Characters are compared based on their ASCII code. A smaller ASCII value means one smaller than another. So numbers are less than capital letters, which are are less than lowercase letters.

In lexicographic order, we compare the characters one by one. If the character at the same index in both strings is the same, we move to the next character and try again. If one is less than the other, then that string is **lexicographically less** than the other. If we reach the end of both strings, then they are equal.

If we reach the end of only one string, the shorter string is lexicographically less than the other. This rule isn't really needed because the null terminator is always less than all other characters.
