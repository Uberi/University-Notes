...
There are also other notations for various asymptotic behaviours. For example, $\Omega(n)$ is the asymptotic lower bound, just as $O(n)$ is the asymptotic upper bound, and it means $c \cdot g(n) \le f(n)$ for some $c$ after $n$ is greater than some $n_0$.
