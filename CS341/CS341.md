...
```python
def spanning_tree(vertices, edges, weight_function):
    leaders = list(range(len(vertices)))
    tree_edges = set()
    for edge in sorted(edges, key=weight_function):
...
def spanning_tree(vertices, edges, weight_function):
    ...
    for vertex in vertices - {start_vertex}:
        W[vertex] = weight_function({start_vertex, vertex})
    ...
    while len(tree_edges) < len(vertices) - 1:
        ...
        for vertex in vertices - tree_vertices: # look at the vertices in the rest of the graph
            if weight_function(min_weight_vertex, vertex) < W[vertex]: # new edge has a lower weight than the current one
                W[vertex] = weight_function(min_weight_vertex, vertex)
...
```
def greedy_minimum_spanning_tree(vertices, edges, weight_function):
    spanning_tree = set()
...
```python
def compute_paths(vertices, edges, weight_function, start_vertex):
    visited = {start_vertex}
    min_path_weights = {start_vertex: 0}
    predecessors = {}
    for vertex in vertices - {start_vertex}:
        min_path_weights[vertex] = weight_function({start_vertex, vertex})
    ...
    while len(visited) < len(vertices):
        ...
        for vertex in vertices - tree_vertices: # look at the vertices in the rest of the graph
            if min_path_weights[min_weight_vertex] + weight_function(min_weight_vertex, vertex) < min_weight_vertex[vertex]: # new vertex has lower path weight than the current one
                min_path_weights[vertex] = min_path_weights[min_weight_vertex] + weight_function(min_weight_vertex, vertex)
...
```python
def shortest_path(vertices, edges, weight_function):
