...
Note that breadth first search, since it visits every vertex and could potentially visit every edge, takes $\Theta(\magn{V} + \magn{E})$ worst case time.
...
def depth_first_search(vertices, adjacency_list):
    ...
    for vertex in vertices:
        if vertex not in visited:
            visit(vertices, adjacency_list, vertex, visited)

def visit(vertices, adjacency_list, vertex, visited):
    visited.add(vertex)
    for neighbor in adjacency_list[vertex]:
        if neighbor not in visited:
            visit(vertices, adjacency_list, neighbor, visited)
```
