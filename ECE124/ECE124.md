...
As a result, it **only updates its output value on the falling edge of CLK**. In reality, there is a short period of time before and after the clock signal goes from 1 to 0 in which D must be stable. This is because there is a setup time $T_{su}$ for the master latch to get its value before the transition, and some hold time ($T_H$) for the master latch to lock in its value). There is also a delay for the value of the master latch to get propagated to the output, the clock to output time $T_{CO}$.
