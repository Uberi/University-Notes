...
Since we have 5 states, we will need at least 3 flip-flops, since they can represent up to 8 states. For each state, we need to assign a set of flip-flop values. For now, we will assign them directly as $0, 0, 0$ up to $1, 0, 1$. However, different assignments could potentially make the circuit simpler.
