...
An $n$-bit **binary ripple counter** counts from $0$ to $2^n - 1$, then wraps around and repeats. We can actually implement this using T-flip-flops that are chained together - the inverted output of one goes to the input of another, and the next bit flips whenever the current bit transitions from 1 to 0, and all the $T$ inputs are set to 1. This works because of the period of a bit $i$ is $2^i$, and each time we use a T-flip-flop, we increase the period by a factor of 2.
