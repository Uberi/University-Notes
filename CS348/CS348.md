...
The **closure of a set of functional dependencies** $F$ is the set of all the functional dependencies it logically implies - $F^+ = \set{X \to Y \middle| F \models X \to Y}$. Saying $f \in F^+$ is equivalent to saying $F \models f$.

The **closure of a set of attributes** $X$ is the set of all attributes that functionally depend on $X$ - $X^+ = \set{A \middle| X \to A \in F^+}$.
...
* Every functional dependency in $F$ is of the form $X \to \set{A}$ where $A$ is an attribute (all the right hand sides are single attributes).
* No functional dependencies can be removed without changing the cover - the number of functional dependencies is minimized.
* There does not exist a $X \to A$ and a $Z \subset X$ such that $F - \set{X \to A} \cup \set{Z \to A}$ covers $F$ - $X$ is the smallest possible set of attributes.
