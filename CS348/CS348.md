...
We can represent a **relation** $R$ over $n$ attributes as a set of $n$-tuples $\tup{a_1, \ldots, a_n} \in D_1 \times \ldots \times D_n$. $\tup{a_1, \ldots, a_n}$, where $D_1, \ldots, D_n$ are the **domains**, sets of simple, single-valued attributes - $D_i$ is the set of all values of attribute $i$. In other words, relations are defined over attributes.
