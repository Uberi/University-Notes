...
1. Generate a **precedence graph** $G$ for the schedule.
    ...
    * In other words, an edge $\tu{T, T'}$ means that $T'$ has to wait because of $T$ locking a data item.
...
Non-repeatable reads can't occur even without predicate locks, but it can occur if we don't use locks.
...
* `READ UNCOMMITTED`: transactions can read data at any point - non-repeatable reads, dirty reads, and phantom reads all possible. Reads ignore locks entirely, including write-locked rows.
...
Wrttes (update/insert/delete) always need to hold long duration predicate write locks (regardless of isolation level).
