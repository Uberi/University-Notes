...
The **scheduler** accepts these pairs of arrival and run times (or estimates of run times), and outputs the times that we should start each of the inputted jobs at. We can characterize different scheduling algorithms by their **response time** characteristics (how long does it take from the job to arrive to when it starts?), and their **turnaround tim** characteristics (how long does it take from the job to arrive to when it finishes?).
