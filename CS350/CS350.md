...
In SYS/161, we control each device by writing to their command/control registers (for controlling the device), status registers (for querying the device status), and data registers (for transferring data between the processor and the device). These registers are read/written to by the hardware mapping them to specific physical addresses on the LAMEbus, and then reading/writing to those addresses.
