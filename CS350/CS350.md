...
1. User-mode program makes the syscall.
    ...
    5. The program counter is set to the fixed syscall address.
...
Midterm average was in the 60's. We're going over many of the questions in the midterm that a lot of people got wrong.
...
A good way to priority scheduling is to use **multi-level feedback queues**. In this scheme, we distinguish between interactive applications (which tend to run for short bursts and sleep most of the time due to waiting for human input), and non-interactive applications (which tend to run for longer bursts and sleep less often). The idea is to gradually reduce priority of threads that behave like interactive applications.
...
A **bus** is a communication path between different devices. Most computers have internal buses (for communication between processor and RAM), and peripheral buses (for other devices like a network card or disk drives). Different buses can be connected with a bus bridge.

Generally, the CPU is connected to the memory bus, which is connected to the RAM and a bridge to the peripheral bus. Newer architectures try to speed up peripherals like GPUs by adding a GPU bus directly between the processor and GPU.