...
Basically, whenever we do a write, we write all the data blocks, and then write an inode entry for those data blocks. When we need to overwrite any data, we just write the new data, and then add a new, duplicate inode entry after that.

Log-structured filesystems are also nice for wear-levelling, because it tends to spread out writes over all the blocks.