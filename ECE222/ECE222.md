...
In addition to arithmetic instructions like `ADD` and `SUBTRACT`, we also have logical operations like `AND DEST, A, B`, `OR DEST, A, B`, `SHIFTL DEST, VALUE, AMOUNT`, `SHIFTR DEST, VALUE, AMOUNT`, `ROTATEL DEST, VALUE, AMOUNT`, `ROTATER DEST, VALUE, AMOUNT`. Rotating without carry rotates ignoring the carry bit, but the bit that corresponds to it is still stored in the carry bit.
