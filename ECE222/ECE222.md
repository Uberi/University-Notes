...
To make a call, we store the value of PC to the link register, and branch to the target of the call. To return, we simply jump back to the saved address. These operations are done using the `CALL SUBROUTINE` and `ROUTINE` opcodes.
