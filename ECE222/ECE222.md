...
To implement this, we split up the virtual memory space into **pages**, each of a fixed size (like 4 kB). Each page can be stored in either the main memory, or swapped to the hard disk. This is sort of like caching, in that we have to choose between two different storage locations, but there's only one copy of every page stored at any time.
