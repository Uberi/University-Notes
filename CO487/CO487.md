...
Instead, we can append the key to the message before we hash it, rather than prepending it. This ensures that the attacker can't use a length extension attack anymore, but it requires the hash function to be collision resistant in order for the MAC to secure. This is called the **secret suffix method**.

If the hash function isn't collision resistant, then we can efficiently find a collision $\tup{x_1, x_2}$. Then $x$ ;wip: collision attack

To avoid the need for collision resistance in the hash function, we can use the **envelope method**, in which we prepend and append the key to the message before we hash it. This successfully avoids the length extension and collision attacks.
...
To send a message from user U to user V (naively; real implementations need to handle things like side channel attacks):

1. U obtains an authentic copy of V's public key $\tup{n, e}$.
2. U represents the message as $0 \le m \le n - 1$ - the message is represented as a single, fixed-size number.
3. U computes $c = m^e \mod n$.
4. U sends $c$ to V.
5. V computes $m = c^d \mod n$.

Decryption is essentially computing $(m^e)^d = m \mod n$. In general, $(m^e)^d$ would have multiple values, because 

Suppose $p \mid m$. Then $m \equiv 0 \pmod p$, so $m^{ed} \equiv 0 \pmod p$, so $(m^e)^d = m \pmod p$.

Suppose $p \mid m$ ;wip: does not divide symbol. Since $ed \equiv 1 \pmod \phi(n)$, $ed = 1 + k\phi(n) = 1 + k(p - 1)(q - 1)$ for some $k$. By Fermat's Little Theorem, $m^{p - 1} \equiv 1 \pmod p$, so taking both sides to the power of $k(q - 1)$, we get $m^{k(p - 1)(q - 1)} \equiv 1^{k(q - 1)} \pmod p$. Multiplying both sides by $m$, we get $m^{1 + k(p  -1)(q - 1)} \equiv m \pmod p$, or $m^{ed} \equiv m \pmod p$. By a similar argument, $m^{ed} \equiv 1 \pmod q$. Since $p$ and $q$ are coprime (since they're distinct primes), $m^{ed} \equiv m \pmod{pq}$, so $m^{ed} \equiv m \pmod n$.
