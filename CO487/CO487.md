...
1. U randomly selects 2 large distinct primes $p, q$ of the same bit length $k$. They're large so their product is hard to factor, and they're distinct so they can't just be factored by square rooting the product. A good value of $k$ is 1024, 2048, or 4096.
...
One way of solving the discrete logarithm problem is **exhaustive search**: Compute $g^i \mod p$ for every $0 \le i \le q - 1$, until we find the $i$ such that $g^i = h$. This takes $q$ modular multiplications, so $O(p)$ runtime (since $p$ and $q$ are the same size).

Shanks's Algorithm is a faster way:

> Clearly, $a = im + j$ for $m = \ceil{\sqrt{q}}$, $0 \le i \le m - 1$, and $0 \le j \le m - 1$. If we find $i$ and $j$, we can find $a$.  
...
The **set of points** on for an elliptic curve is the set of solutions to the equation itself, plus the point at infinity. For this curve the set of points on the curve is $E(F) = \set{\tup{x, y} : y^2 = x^3 + ax + b} \cup \set{\infty}$. Clearly, $E(F) = \set{\tup{0, 3}, \tup{0, 10}, \tup{1, 5}, \tup{1, 8}, \ldots}$.
...
We'll call $P$ a **generator** of $E(\mb{Z}_p)$. For example, for $E(\mb{Z}_{13}) : Y^2 = X^3 + 2X + 9$, $#E(\mb{Z}_{13})$, we let $P = \tup{0, 3}$, and get $E(F) = \set{\tup{0, 3}, \tup{3, 9}, \tup{1, 8}, \tup{11, 7}, \ldots, \tup{11, 6}, \tup{1, 5}, \tup{3, 4}, \tup{0, 10}, \infty}$.
...
* The group $E(\mb{Z}_p) = \set{\infty, P, 2P, \ldots, (q - 1)P}$ rather than $<g> = \set{g^0 \mod p, \ldots, g^{q - 1} \mod p}$, both with size $q$.
