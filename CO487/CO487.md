...
First, we pick an arbitrary message $m$, and compute $h = \mathrm{SHA1}(m)$. Let $z$ be the byte 0x00. Let $C$ be the 15-byte name of SHA1 in the PKCS #1 V1.5 standard. Let $D = z \| C \| h$ be the formatted version of our fake message (we don't use the FF padding here).
