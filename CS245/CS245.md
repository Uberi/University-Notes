...
If $\Sigma$ is satisfiable, then $\Sigma$ has a **model**.
...
The proof format we have been using so far is in something known as **sequent notation**. The $\vdash$ symbol is a sequent, with all premises before it and all conclusions after.
...
**Syntactical consistency** is a property of sets of formulas $\Sigma \subseteq \formlp$ where there does not exist $\alpha \in \formlp$ such that $\Sigma \vdash \alpha$ and $\Sigma \vdash \neg \alpha$ at the same time - that it not possible to derive a contradiction from the premises.
