...
If $y$ is a variable in $t$, then if there are no occurrences of $x$ in the scope of $\exists y$ or $\forall y$ in a formula $A$, then $t$ is **free for** $x$ in $A$. Basically, $t$ is free for $x$ in $A$ if substituting $t$ for any occurrence of $x$ does not cause any free variables to become bound.
...
> Let $I = \tup{\mb{N}, \begin{cases} 3 &\text{if } a \\ 4 &text{if } b \\ x + y &\text{if } f(x, y) \\ x^2 &\text{if } g(x) \\ = &\text{if } P \end{cases}}$.  
...
| Universal-elimination  | $\forall-$ with $\Sigma \vdash \forall x. \alpha$ and $t$ is a constant           | $\Sigma \vdash \forall x. \alpha$ implies $\Sigma \vdash \alpha[x/t]$                             | If $\alpha$ is true for all $x$, then it is true for a particular $x$ as well. |

Show that $\forall x. \forall y. P(x, y) \vdash \forall y. \forall x. P(x, y)$:

1. $\forall x. \forall y. P(x, y) \vdash \forall y. P(u, y)$ for arbitrary $u$ ($\forall-$)
2. $\forall x. \forall y. P(x, y) \vdash P(u, v)$ for arbitrary $v$ ($\forall-$)
3. $P(u, v) \vdash \forall y. P(u, y)$ ($\forall+$)
4. $P(u, v) \vdash \forall x. \forall y. P(x, y)$ ($\forall+$)
5. $\forall x. \forall y. P(x, y) \vdash \forall x. \forall y. P(x, y)$ ($\operatorname{Tr}$)
