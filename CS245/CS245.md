...
We can prove a set $S$ is not countable by proving that the bijection cannot exist, usually by showing there is a bijection $f: U \to \mb{N}$ where $U \subset S$, so there cannot be a bijection between natural numbers and the entire set because the elements $S \setminus U$ cannot be mapped into $\mb{N}$ while making sure $f$ is one-to-one.
