...
We can write $A, B \vdash C$ as $\frac{A \qquad B}{C}$. This is the inference notation.

### Rules

Every rule has a name that we use when we reference it by when applying it.

Reflexivity (Ref): $A \vdash A$ - $\frac{A}{A}$.

Addition of premises (+): given $A \vdash C$, $A \cup B \vdash C$.

;wip: all the way until ND - Rule of Negation

Using the sequent ($\vdash$) notation, we must always write all the premises we use to deduce anything on the same line it is being deduced. For example:

1. $p \vdash \neg \neg p$
2. $q \vdash p \rightarrow q$
3. $\neg \neg p, p \rightarrow q \vdash q$.

In Box Notation, we write assumptions, and then we draw a box that contains it, the box acts like a "scope" in which the assumptions hold and we can directly write the deduced thing. Inside the box we can directly write the resulting formulas without writing the $\alpha_1, \ldots, \alpha_n \vdash$ before it. Scopes can nest, and this saves a lot of rewriting of premises. For example:
