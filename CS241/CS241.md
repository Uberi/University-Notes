...
The `print.merl` module will print the value in register 1. To generate code for `println(expr);`, we evaluate `expr`, set the result as the parameter for `print`, and call it. If we are using register 1 before that, then we need to save it somewhere first:
