...
We often just use a simpler version of maximal munch, which works on a more limited set of languages:
...
Basically, the same thing, but without the backtracking step. This means we can't parse rules like "identifiers must start and end with a letter, but may contain the `--` operator". However, this is usually good enough because languages are often designed intentionally in such a way that this works.
...
;wip: midterm covers everything up to this point
...
The step where we find which production rule to use is an important part of top-down parsing. Generally, this is based on 
...
| Stack      | Read input | Unread input | Action                                                     | Current Input | ;wip: handle \vdash and \dashv
|:-----------|:-----------|:-------------|:-----------------------------------------------------------|:--------------|
| $S$        | $\epsilon$ | `abywx`      | (initial state)                                            | $S$           |
| $AyB$      | $\epsilon$ | `abywx`      | top of stack is nonterminal, pop and pick rule $S \to AyB$ | $AyB$         |
| $abyB$     | $\epsilon$ | `abywx`      | top of stack is nonterminal, pop and pick rule $A \to ab$  | $aayB$        |
| $byB$      | `a`        | `bywx`       | read `a`, compare to top of stack, pop | $abyB$            | $aayB$        |
| $yB$       | `ab`       | `ywx`        | read `b`, compare to top of stack, pop | $abyB$            | $aayB$        |
| $B$        | `aby`      | `wx`         | read `y`, compare to top of stack, pop | $abyB$            | $aayB$        |
| $wx$       | `aby`      | `wx`         | top of stack is nonterminal, pop and pick rule $B \to wx$  | $abywx$       |
| $x$        | `abyw`     | `x`          | read `w`, compare to top of stack, pop                     | $abywx$       |
| $\epsilon$ | `abywx`    | $\epsilon$   | read `x`, compare to top of stack, pop | $abywx$           | $abywx$       |
| $\epsilon$ | `abywx`    | $\epsilon$   | end of input, stack empty, so input valid | $abywx$        | $abywx$       |
