...
We can also do **constant propagation**, in which we propagate constant values like `int x = 1` ahead into the code, so if we do a `1 + x` later without changing `x` first (or modifying it indirectly via pointers), we can substitute it for `1 + 1` (and now we could potentially do constant folding here).