...
It turns out the Simplex algorithm already finds cutting planes.
...
> Since $f(\vec x)$ is a convex function, $f(\vex y) \le kf(\vec a) + (1 - k)f(\vec b)$. Clearly, $kf(\vec a) + (1 - k)f(\vec b) \le k\beta + (1 - k)\beta = \beta$.  
