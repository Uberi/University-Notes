...
A **basic solution** for an LP and a basis $B$ is a solution $\vec x$ for the LP such that $A \vec x = \vec b$ and all non-basic variables are set to 0. For the basic solution for a basis $B = \set{b_1, \ldots, b_k}$, the LP becomes "maximize ${\vec c}^T \vec x$ subject to $A_B \begin{bmatrix} x_{b_1} \\ \vdots \\ x_{b_k} \end{bmatrix} = \begin{bmatrix} r_{b_1} \\ \vdots \\ r_{b_k} \end{bmatrix}$".

Since $B$ is a basis, $A_B$ must be invertible, so $A_B^{-1} A_B \begin{bmatrix} x_{b_1} \\ \vdots \\ x_{b_k} \end{bmatrix} = A_B^{-1} \begin{bmatrix} r_{b_1} \\ \vdots \\ r_{b_k} \end{bmatrix}$, or $\begin{bmatrix} x_{b_1} \\ \vdots \\ x_{b_k} \end{bmatrix} = A_B^{-1} \begin{bmatrix} r_{b_1} \\ \vdots \\ r_{b_k} \end{bmatrix}$. Interestingly, this means that **a basis $B$ has exactly one basic solution**, and that solution is a vector $\vec x$ where ${\vec x}_B = A_B^{-1} {\vec b}_B$ and ${\vec x}_{\overline B} = \vec 0$.
...
Suppose we have the LP in SEF "maximize $\begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} \vec x$ subject to $\begin{bmatrix} 1 & 0 & 1 & -1 \\ 0 & 1 & 1 & 2 \end{bmatrix} \vec x = \begin{bmatrix} 1 \\ 2 \end{bmatrix}$ and $\vec x \ge \vec 0$". One basis for this LP is $B = \set{2, 3}$ - it's a square matrix, and its columns are linearly independent.
...
We'll add the objective function to both sides of that, to get $\begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} \vec x + {\vec y}^T \begin{bmatrix} 1 & 0 & 1 & -1 \\ 0 & 1 & 1 & 2 \end{bmatrix} \vec x = \begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} \vec x + {\vec y}^T \begin{bmatrix} 1 \\ 2 \end{bmatrix}$. Rearranging, we get $\begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} \vec x = \left(\begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} - {\vec y}^T \begin{bmatrix} 1 & 0 & 1 & -1 \\ 0 & 1 & 1 & 2 \end{bmatrix}\right) \vec x + {\vec y}^T \begin{bmatrix} 1 \\ 2 \end{bmatrix}$.

Clearly, the left side is the objective function, and it's equal to the right side. However, we can manipulate $\vec y$ until the value that's being multiplied with $\vec x$ has zero entries for our basic variables! Basically, we want to choose $\vec y$ such that ${\vec y}^T \begin{bmatrix} 1 & 0 & 1 & -1 \\ 0 & 1 & 1 & 2 \end{bmatrix}_{\set{2, 3}} = \begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix}_{\set{2, 3}}$. In other words, $y_1 0 + y_2 1 = 0, y_1 1 + y_2 1 = 2$. Solving this system, we get $\vec y = \begin{bmatrix} 2 \\ 0 \end{bmatrix}$.

Now $\begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} \vec x = \left(\begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} - {\vec y}^T \begin{bmatrix} 1 & 0 & 1 & -1 \\ 0 & 1 & 1 & 2 \end{bmatrix}\right) \vec x + {\vec y}^T \begin{bmatrix} 1 \\ 2 \end{bmatrix}$ becomes $\begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} \vec x = \left(\begin{matrix} 0 & 0 & 2 & 4 \end{bmatrix} - \begin{bmatrix} 2 & 0 & 2 & -2 \end{bmatrix}\right) \vec x + 2 = \begin{matrix} -2 & 0 & 0 & 6 \end{bmatrix} \vec x + 2$. So for feasible solutions, the objective function is exactly equal to $\begin{matrix} -2 & 0 & 0 & 6 \end{bmatrix} \vec x + 2$.
