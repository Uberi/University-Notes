...
One way to do write-back caching is the MESI protocol. In this protocol, caches are divided into blocks, and each block is either:

* SHARED: local value is up to date (clean), multiple copies exist, and read-only.
* EXCLUSIVE: local value is up to date (clean), only local copy exists.
* MODIFIED: local value is out of date (dirty), only local copy exists.
* INVALID: local value is out of date (dirty), must reload this block before using it.
