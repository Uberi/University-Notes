...
The **CRUM** (Computational Representational Understanding of Mind) paradigm is another name for the central ideas of classical cognitive science - that thinking is like computation over knowledge stored as representations, that intelligence is defined in terms of knowledge, and that intelligence can be tested for with computational models and various other known methods.
...
Another criticism of pictorialism is that it's unfalsifiable: the definition of a visual mental image is defined so loosely that any experimental result could be interpreted as support of it. In response, a proponent of pictorialism might say that the difficulty in falsifying is due to the inherent complexity of the problem itself and in obtaining good evidence for any position on the problem, and it should be expected to be difficult to falsify even if it was the best theory ever. The pictorialism view is computationally equivalent to the descriptionism view. However, pictorialism makes certain testable empirical claims about the structure of the human brain that may support or falsify it in the future.
...
Classical cognitive science has often thought of cognition in terms of a **perceive-think-act cycle**, where agents perceive their environment, plan out the best response, and then actually perform the action. However, humans can't do this - the thinking/planning would take so long that we wouldn't be able to do any realtime tasks at all.

Additionally, many models think of sense input as infallible, while human perception is generally only partial and incomplete. Cognitive science needs models that depend less on having full knowledge of the situation, and move the action and perception closer together in the cycle.

For example, when catching a baseball, if we tried to perceive the coordinates of the ball and compute the trajectory before moving to catch the ball, we would never get there in time to actually catch it. Instead, most humans try to move toward a constantly updating estimate of where the ball will land. Contrast this with chess, where we have perfect information about the state of the game, and therefore can often dedicate time specifically to planning.

Instead of cognition as the thinking in the perceive-act-think cycle, some cognitive scientists think that intelligence is a tool that organizes interactions between the agent's body and the world, rather than a tool that helps plan the best way for the agent to act.

The original theory of visual perception said that perception is like abductive inference - what we actually perceive is the best interpretation of what we're seeing. However, Gibson proposed a different theory, **direct perception**, which said that rather than perception generating representations of the world and knowledge about the agent's position within the world, the actual data coming in from the senses (as well as things like changes in that data) have all the information needed to do many different cognitive tasks. Gibson demonstrated this when he found that pilots tended to judge their position for landings through things like texture and motion rather than perspective or shadows. In other words, the optic flow is all that is needed to judge the landing, no mental representations necessary.

According to Gibson, the things we perceive are linked to action in ways that don't suggest that we're making any mental representation. People often try to push doors that say "Pull" on them, simply because their visual perception saw that the handle afforded pushing more than pulling, rather than reading the text and making inferences. The optic flow suggests what we might interect with different objects - when we see an object, we're finding ways to interact with that object rather than constructing mental representations of that object.

Direct perception does allow us to move directly from perception to action, with no thinking in between, which does model a lot of real human activities pretty well. Under this model, expertise is the state of having refined perceptions rather than having a greater number of concepts or rules - a professional pianist sees more and higher quality affordances than an amateur. In Gibson's model, there's really no such thing as knowledge, in the rules and concepts sense.

This does not mean that there are no mental representations, however. Going by the concept model of cognition, frames for different concepts do contain things like different interactions that should be performed - we could think of affordances as different tasks stored in the frame for what we're perceiving.

Classical cognitive science is mostly focused on representing knowledge rather than how agents take action.

Johnson argues that embodiment (having a body) is important for cognition because it is used to generate metaphors and analogies that are used to understand concepts in the world. For example, when we see an airplane, we understand that it's pushing through the air because we can also feel the air when our legs push us forward.

An image scheme is a recurring pattern in perception and bodily activity. For example, when we push ourselves forward with our legs, we perceive the air being pushed aside. These image schemes can be thought of as a sort of knowledge, and they can be applied to our thinking, like the concept of our bodies being pushed into other objects being pushed, and even into more abstract things like being socially pushed into doing things.

According to Johnson, the motor cortex contains neural paths that represent knowledge of how to take certain actions or how to respond to certain events. Because classical cognitive science generally thinks in terms of disembodied agents, models often end up replacing image schemes with abstract concepts. This is a mistake, Johnson says, because abstract concepts representing patterns in perception/bodily activity aren't as significant as image schemes to the agent, and abstract concepts take a lot more work to apply than image schemes when trying to perform a given task using a body. In other words, image schemes are an important optimization for embodied agents.

Shakey, a robot built in the 1970s, was one that used a classical cognitive science paradigm, keeping track of a lot of mental representations and procedures to get an accurate model of the world. However, it was incredibly slow. The classical cognitive science robot architecture keeps an internal representation of the world, and an executive unit constantly tries to sync changes in the world to the internal model through sensors, and sync changes in the internal model to the world through actuators. One way to get around this is to build robots that just respond directly to sensor input without an internal model.

According to Rodney Brooks, we can achieve intelligence without internal models through **situation** and **embodiment**. By situation, he means that instead of keeping an internal model of the world, just use perception of the actual world instead to shorten the perceive-think-act cycle into just the perceive-act cycle. By embodiment, he means that agents need the right kind of body that joins perception and action effectively.

Brooks says it is possible to build an intelligent agent using a subsumption architecture, and that this is capable of human-level intelligence. In this architecture, there are many different perception/action controllers that map perceptions into actions, and they're linked up in a decentralized network where some nodes can perceive or act on the world.

One criticism of this is that the subsumption architecture doesn't scale up, because true intelligence may require a centralized controller for perception and action. Also, the subsumption architecture hasn't been able to explain any actually intelligent behaviour yet, only simple ones.

Dynamical Systems
-----------------

Some cognitive scientists reject classical computer science, and instead propose **dynamical systems theory**, which tries to explain cognition in terms of neural activity patterns between many interacting systems. A dynamical system is a system which changes its own behaviour over time, like clocks, the solar system, and computers.

Van Gelder says that intelligence is about interacting properly with the environment, and the digital computer model of cognition doesn't satisfy the interactivity requirement. Basically, instead of abstracting away the details of the interactions between different systems working together, we should study these interactions in more detail to gain insight into cognition.

Dynamical systems are generally influenced by positive feedback (changes cause increasing changes, tends to run away by itself) and negative feedback (changes cause decresing changes, tends to settle down by itself). The interaction between different feedback loops can result in interesting behaviours.

An **attractor** is a series of states of a system that nearby states tend to evolve toward. An **fixed-point attractor** is an attractor consisting of a single state, while **periodic attractor** is an attractor consisting of a cycle of states. For example, for a ball rolling down the side of a valley, the lowest point in the valley can be considered a fixed-point attractor, since the ball will tend to stay there after a while.

When a system changes from being influenced by one attractor to another, this is known as a phase shift/transition. We can think of a dynamic system's behaviour as a function from time to system state; dynamical system modelling focuses on identifying attractors in that state function, and analyzing behaviour in terms of those attractors.

Dynamical system theory says that the human brain doesn't really contain anything like mental representations - it's just part of the operation of the system, like a lever or a gear. In other words, cognition is not the result of computation on mental representations, but rather a very large and complex dynamical system.

Modelling emotions in relationships as dynamical systems has had promising results. By representing how one spouse influences the other's emotions through conversation as influence functions, and representing the uninfluenced steady state of each person as the initial state, we can think of emotions in the relationship as a dynamical system. Analyzing feedback loops and attractors of positive and negative emotions yielded new insight into which factors dominate the emotional tone of each relationship. It turned out that the main factor in successful relationships was how willing each person was to be emotionally influenced by the other.

One criticism of the dynamical systems theory is that there's no way we could analyze or even figure out the equations for a system as complex as the brain, and any simplification wouldn't really capture cognition at all, since many things in the brain's dynamical system are chaotic. Another is that dynamical systems theory focuses on different things - we might use dynamical systems theory to study the motor cortex or visual cortex, but classical cognitive science to study things like high-level cognition.

Additionally, they aren't necessarily incompatible, as a sufficiently advanced dynamical system made of neurons can emulate a computer, and a computer can emulate a dynamical system - the brain being a dynamical system doesn't mean it can't have mental representations. Research into recurrent neural networks often make use of concepts from dynamical systems theory to describe how the entire network behaves during activity propagation, even though these networks do have distributed representations.
...
We generally think of people as knowing things, but not books or search engines. What's the difference? One theory is that the person's mental representations are qualitatively different from the textual representations in the book or computer. This quality is known as **intentionality**, and is the property of being able to possess knowledge. The problem of intentionality is "what is intentionality, really?".

This is an important problem for classical cognitive science, since it determines what we consider to be able to know things. If our computer models of cognition can't ever know things, then classical cognitive science can't accurately model cognition.

According to Brentano, intentionality is the quality of being **about** something or **meaning** something - a person's mental states are intentional because they're about something else. However, what does it mean for something to be about something else? It can't just be that the other thing caused this thing, because something can be about something else that doesn't even exist.

**Phenomenology** is the philosophical approach of looking at mental states independently of how they are linked to the material world - studying conscious experience by itself. Phenomenology says that reality is the perception of phenomena in the human consciousness, not anything independent of human consciousness.

According to phenomenology, existance is the process/activity of being present (this concept is known as Dasein). Therefore, the mind is not a machine, but rather an instrument for existing. Phenomenology says that knowledge is not information about how things are, but rather information about how to different things. Additionally, knowledge is holistic, so it can't be split into small pieces and considered in isolation.

Since knowledge is holistic and about how things are done, it can't be made explicit into things like rules or concepts, which challenges classical cognitive science's view that knowledge can be represented that way. In this view, we can't build a machine that knows things by manipulating symbols.

Dreyfus presents a phenomenalistic account of cognition, where all intelligent behaviour is the result of intentional qualities of our mental states, refined through practice. In this view, knowledge is a holistic thing that helps us interact with the world. As a result, it answers the problem of intentionality by saying that our knowledge is about how to interact the world, while what we have in books is only about how the world is - intentionality is the quality of being linked somehow to interacting with the world, so **knowledge is mental states/representations that are linked to perception and action**.

But what if we built a robot that linked information to its sensors and actuators? Would it then have knowledge, because it contains mental states linked to its perception and action?

**Strong AI** is the idea that a simulation of a human brain can think. Searle thinks strong AI is false - a simulation of a fission reaction doesn't give off any radiation, just as a simulation of thinking doesn't give off any thoughts. To justify this, Searle presents the Chinese room argument, which tries to show that a simulation on a computer can't really have true mental states - a person's mental states have meaning, while symbols on a computer are by nature meaningless. The argument goes like this:

> Suppose someone who doesn't know Chinese is locked in a room with a rulebook that lets them correctly answer Chinese questions using blind symbol manipulation. The man does not understand Chinese, yet the room behaves exactly as if it contains someone who does. If the man doesn't understand Chinese, and the rulebook doesn't understand Chinese (because it's a book), then nothing in the room really understands Chinese.  
> Likewise, a digital computer doing blind symbol manipulation can't understand anything, even if it outwardly appears exactly as though it does.  

According to Searle, a mental state requires understanding. So if the robot can't have understanding, then it doesn't really have mental states, and therefore doesn't have knowledge/thoughts either. Searle thinks for some reason that only human mental states are intentional/meaningful, and therefore only humans have knowledge.

The functionalist response to the argument is that the room as a whole system does in fact understand Chinese, even if the components don't. Searle responds to this with another thought experiment: suppose the person in the room memorized the book and left the room - they still don't understand Chinese, yet can outwardly behave exactly as if they do! Therefore, we can't say something has understanding even if they outwardly behave exactly the same as if they did.

The interactionist response to the argument is that a mental state is meaningful if it's created/updated by interactions with the real world. Searle responds to this with another thought experiment: suppose we replaced the man in the room with a machine that perceived questions from sensors, translates it internally using various rules, and then output answers - the machine doesn't understand Chinese, even if the room behaves exactly as if it contained someone who did. Therefore, we can't say something has understanding even if their representations are formed and updated based on interactions with the world.

The holistic response to the argument is to respond with another thought experiment: suppose we have a simulation of a human brain that is indistinguishable from a real one. Clearly, the simulation understands Chinese, or else we might as well say physical brains can't understand Chinese either. The holistic model says that meaningfulness comes from similarity to the human brain. Searle maintains that meaningfulness is separate from behaviour or representation, however, and responds to this argument with yet another thought experiment: if the room contained a man who doesn't understand Chienese, operating a machine that simulated a full human brain that does understand Chinese, the man still wouldn't understand Chinese. For some reason, this thought experiment doesn't entertain the possibility of the simulation itself understanding anything, which is kind of circular - if we assume that simulations can't understand things, then yes, simulations can't understand things.

The combination response to this argument tries to combine the above three responses into one counterargument: suppose we have a simulation of the brain that uses holistic, brain-like representations, which work exactly like an actual human brain, and the simulation is placed in a robot body that can interact with the world to update the simulation with. Basically, the combination response assumes that meaning/intentionality is when a mental state is affected in the right way through interaction with the world, and the mental states use holistic, brain-like representations. However, Searle says that the robot in that case would still not really be thinking, only appearing to, because meaning is separate from anything about behaviour or representation. Searle adds that since the program running the simulation doesn't think, neither does the simulation itself.

Searle's position is rather difficult to defend. The other minds response says that under Searle's criteria, other humans aren't capable of thought either, because if we understand the biology of the brain, surely we could say that individual neurons can't think, so the brain can't think either. Searle could either be a dualist (there's some sort of immaterial nature of human brains that gives rise to meaning, a sort of soul for cognition) or a materialist (the exact biology and structure of the brain is what gives it the ability to generate meaning).

Searle presents the **biological naturalism** account of intentionality - the actual physical properties of the neurons and synapses is what allows the brain to have intentional mental states, though Searle can't articulate what they are, or why artificial circuits couldn't have the same properties. Searle also suggests that it is the causal powers of the brain, though this doesn't make any sense since in theory, a computer could be programmed to do anything the brain could do.

According to Searle, if the neurons in the brain were gradually replaced by functionally identical circuits, at some point the brain would suddenly no longer be able to be intentional. Searle doesn't identify which point that is. Since symbol-shuffling circuits can theoretically be made to simulate any of the physical properties of neurons and synapses to an arbitrarily high fidelity, Searle's view that it's impossible for symbol-shufflers to have intention is also indefensible. Searle instead seems to be basing his arguments off of **homocentrism** - attempting to work backwards from the conclusion that only human-like things that we're intuitively familiar with can have minds, while everything that we're not used to cannot.

Externalism
-----------

Knowledge seems to be more than just facts stored within the mind of an individual. For example, people working in teams can accomplish things that no individual could by themselves - the interactions between people seem to play a role in cognition. **Externalism** is the view that knowledge/intelligence rely not only on the individual, but also on objects and other individuals in the environment.

In contrast, classical cognitive science is internalist, where knowledge exists only in the minds of individuals (a concept called epistemic individualism) and the actions of a group of people is just the sum of the actions of its members (reductionism). So classical cognitive science says that if a group knows something, then at least one individual in the group must also know it. Externalism challenges this individualist concept of intelligence.

However, this internalist view doesn't seem to be the case. You might "know" what something means by being able to look it up in a dictionary, even if you can't recall it off the top of your head. We might not know what the true value of a stock might be individually, but over an entire market we can often know the value to a very accurate degree. Often, the average estimates in a group are better than any individual estimate by members of the group. As a group and as part of our environment, we seem to be able to be more intelligent than any particular individual.

Thomas Hobbes presents a non-reductionist view of group intelligence, and justifies this by looking at societies - people are naturally at war with the world, but in groups they agree to social contracts to form civil societies, so their group behaviour is very different from individual behaviour. Many examples of this can also be found in nature, such as the emergent intelligence of swarming animals like ants and bees.

Cognitive technologies are commonly used to augment our brains. For example, writing can be used to increase our memory capabilities, diagrams can allow us to visualize things we otherwise would have a hard time imagining, and long division can help us perform mathematical operations that we otherwise couldn't. It could be said that the person plus the technology is more intelligent than just the person by themselves.

One instance of this was demonstrated when subjects were shown a dual-interpretation image (such as the famous one that can either be a candlestick or two faces), and asked to draw it rotated. While they could easily do so when able to look at the image, they had much more difficulty doing so when required to draw them from memory.

Note that cognitive technology is different from situatedness (the world being the best model of itself), because in situatedness the mind is still contained in the individual and it just uses the world to model interactions with the world, while with cognitive technologies you could say that parts of the world are also included in the mind itself.

This has important effects on how we do science. The traditional, internalist view is that scientific knowledge is reductionist (scientific knowledge is an amount of evidence that could convince any competent scientist that the claim is justified) and individualist (any competent scientist can evalaute whether a claim is justified or not). Externalist accounts of science say that scientific knowledge is instead the informed consensus of the relevant scientific community.

Kuhn's account of knowledge is one example of an externalist view. In this view, the whole scientific community reaches consensus on a particular paradigm that determines the methods and theories scientists work with, then everyone in the community works within that paradigm. In fact, the paradigm (and therefore, the consensus) influences what the views of individual scientists are.

When a large ship needs to dock, no individual member of the crew is actually capable of doing the whole thing, but collectively, we say that the crew knows how to dock the ship - the knowledge is spread out over multiple people. Hutchins presents an account of this distributed cognition where groups of people are like connectionist networks - each person receives inputs from others, and output results that might influence others in the group. As a whole, the group makes decisions through inhibitory/excitary influence over each other. The structure of the group itself (who's in it, what leadership structure is used) can actually be said to contain its own knowledge, and can be said to learn new things through reorganization and changing the roles of individual group members.

Summary
-------

Classical cognitive science has been pretty successful in explaining cognition through computational models. Even if it has issues mapping models to actual brains, it has useful explanatory powers and can be extended to account for these shortcomings.

Fundamentally, classical cognitive science is about thinking as computation - a very functionalist view of cognition. Challenges include accounting for consciousness and emotions, competing theories (like dynamical systems theory) that explain some things better, and unaccounted-for cognitive phenomena (like group intelligence).

One way to fit things like neuroscience into the classical cognitive science framework is to view them as different levels of abstraction. Chemistry and neuron biology would be a low-level description of the brain, neuroscience would be a mid-level description of the brain, and something like the global workspace theory would be a very high-level description of the brain. At the higher levels, we don't need to worry about low-level details like activation patterns in the brain, but then we obtain many possible theories that can explain a given human behaviour.
